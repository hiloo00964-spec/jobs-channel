...
def smart_clean_with_gemini(text):
    """استخدام جيميناي لتنظيف إعلانات الوظائف بذكاء"""
    try:
...
def main():
    print(f"🚀 [نظام الوظائف]: بدء العمل في {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    for src in SOURCES:
        try:
            ...
            for msg_html in reversed(messages[-8:]):
                ...
                sig = clean_text[:80] # بصمة التحقق من التكرار
                if sig in history: continue
                
                print(f"✨ اكتشاف وظيفة في {src}.. جاري المعالجة..")
