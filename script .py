...
# --- الإعدادات (تأكد من وجودها في GitHub Secrets لمستودع الوظائف) ---
...
def main():
    # --- نظام التقارير الحية لـ Logs ---
    print(f"🚀 [نظام الوظائف]: بدء التشغيل في {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    if not os.path.exists(DB_FILE):
        with open(DB_FILE, 'w', encoding='utf-8') as f: f.write("START\n")
        print("📁 تم إنشاء ملف تاريخ الوظائف الجديد.")
    ...
    print(f"🔍 جاري فحص {len(SOURCES)} مصادر للوظائف...")
    ...
    for src in SOURCES:
        try:
            ...
            for msg_html in reversed(messages[-8:]): # فحص آخر 8 منشورات
                ...
                print(f"✨ تم العثور على وظيفة جديدة في {src}.. جاري التنظيف بواسطة Gemini..")
                processed_text = smart_clean_with_gemini(clean_text)
                
                if "إهمال" in processed_text:
                    print(f"🗑️ منشور مستبعد (إعلان قناة أو ليس وظيفة).")
                    continue
                
                if post_to_telegram(processed_text):
                    with open(DB_FILE, 'a', encoding='utf-8') as f: f.write(sig + "\n")
                    print(f"✅ تم النشر في القناة بنجاح.")
                    found_in_source = True
                    time.sleep(10) 
                    break 
            
            if not found_in_source:
                print(f"😴 لا توجد وظائف جديدة حالياً في @{src}")

        except Exception as e:
            print(f"⚠️ خطأ أثناء فحص {src}: {e}")

    print(f"🏁 انتهت عملية فحص الوظائف بنجاح في {datetime.now().strftime('%H:%M:%S')}")
